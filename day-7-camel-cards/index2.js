...
function getHandValue(str) {
    ...
    // // need to take the J value and add it to highest
    if (hand["J"]) {
        const jValue = hand["J"];
        if (jValue === 5) return 6;
        delete hand["J"];

        let highestKey = Object.keys(hand)[0];
        for (const key in hand) {
            if (hand[key] > hand[highestKey]) highestKey = key;
        }

        hand[highestKey] = hand[highestKey] + jValue;
    }
